...
message-header-action-label = Действие
message-header-time-icon = 🕐
message-header-time-label = Время
...
field-traffic-used = Использовано трафика
field-cpu-model = Модель CPU
field-cpu-count = Количество CPU
field-total-ram = Всего RAM
field-ip = IP
...
event-user-header-title = Информация о пользователе
...
event-node-header-title = Информация об узле
...
event-service-header-title = Событие сервиса
